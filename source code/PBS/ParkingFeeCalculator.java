...
public class ParkingFeeCalculator {
	...
	private double parkingFeeWithoutDiscount(int typeOfVehicle, int typeOfCar,
			int dayOfWeek, double apd) {
		double fee = 0.0;
		if (apd <= 0 || apd > 24)
			return -1;
	...
	private double parkingFeeWithDiscountCoupon(int typeOfVehicle,
			int typeOfCar, int dayOfWeek, double apd) {
		return formatDouble(parkingFeeWithoutDiscount(typeOfVehicle, typeOfCar,
				dayOfWeek, apd) * 0.5);
	}

	private double parkingFeeWithEstimation(int typeOfVehicle, int typeOfCar,
			int dayOfWeek, double apd, String estimation) {
		if ((estimation.equals("(0, 2]") && apd > 0 && apd <= 2)
				|| (estimation.equals("(2, 4]") && apd > 2 && apd <= 4)
				|| (estimation.equals("(4, 24]") && apd > 4 && apd <= 24))
			return formatDouble(parkingFeeWithoutDiscount(typeOfVehicle,
					typeOfCar, dayOfWeek, apd) * 0.4);
		else if (!estimation.equals("(0, 2]") && !estimation.equals("(2, 4]")
				&& !estimation.equals("(4, 24]"))
			return -1;
		else
			return formatDouble(parkingFeeWithoutDiscount(typeOfVehicle,
					typeOfCar, dayOfWeek, apd) * 1.2);
	}
	
	public double parkingFee(int typeOfVehical, int typeOfCar, int dayOfWeek, double apd, boolean discountCoupon, String estimation) {
		double fee = -1;
